...
const int pinMT =  2; // Pin D2 Sharp PC-126x, pin 1 on the 11 pin connector.
const int pinBUSY = 4; // Pin D94 Busy Sharp PC-126x, pin 4 on 11 pin connector.
const int pinDout = 5; // Pin D5, Dout Sharp PC-126x, pin 5 on the 11 pin connector.
const int pinXin = 6; // Pin D6 Sharp PC-126x , pin 6 on the 11 pin connector.
...
const int programSize=1000; // for most programs this can be enough, more would probably cause memory issues
  ...
  if (Serial.available() > 0 ) {
    ...
    switch (inputChar) {
      case 10 : // end of file?
        ...
        Serial.println();
        Serial.println();
        Serial.println (F("Now enter CLOAD, press ENTER on the PC126x, press the start button on the board"));
        // initialize a few variables
        segmentString='\0';
        sourceSegment='R'; // S is default, R is row, D is data, T is text
        outputDecimal = 0;
