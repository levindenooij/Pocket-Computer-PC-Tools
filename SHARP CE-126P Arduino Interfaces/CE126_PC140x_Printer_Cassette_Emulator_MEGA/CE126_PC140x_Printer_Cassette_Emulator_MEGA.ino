...
// Program Name : PC140x_CE126_Emulator
/*    
...
// breadboard : Pin A13 -> xOx pushbutton >- LED statuslight > 330 OHM resistor > PIN A15

const int pinMT =  2; // Pin D2 Sharp PC-1401, pin 1 on the 11 pin connector.
const int pinBUSY = 4; // Pin D94 Busy Sharp PC-1401, pin 4 on 11 pin connector.
const int pinDout = 5; // Pin D5, Dout Sharp PC-1401, pin 5 on the 11 pin connector.
const int pinXin =  6; // Pin D6 Sharp PC-1401 , pin 6 on the 11 pin connector.
const int pinXout = 7; // Pin D7, Xout Sharp PC-1401, pin 7 on the 11 pin connector.
const int pinACK = 9; // Pin D9, ACK  Sharp PC-1401, pin 9 on the 11 pin connector.


// print setup
boolean Busy; // busy indicator
boolean Xout; // Xout indicator
...
const int refSize=89; // size of the reference table for PC-1401 commands, verbs and functions
...
int refIndex; // index of  both reference arrays

// program data array setup
const int programSize=2000; // for most programs this can be enough, more would probably cause memory issues
...
void setup() {
 ...
 // start serial and fileserial communication
  ...
  Serial1.begin(9600); // serial monitor for program notifications
...
// have the user start the program.
  Serial.println();
  Serial.println(F("When ready, 1. run LLIST or program with LPRINT or 2. send basic source file via serial input"));
}
...
break;
case 48:
Serial.print("O"); // character "O"
break;
case 240:
Serial.print("0"); // number Null
break;
default:
  ...
  if (Serial1.available() > 0 ) {
    ...
    switch (inputChar) {
      case 10 : // end of file?
        ...
        Serial.println(F("Sourcefile processed!"));
        Serial.println();
        Serial.println(F("This is the generated code with comma's for readability"));
        Serial.println();
        // print the array
        for (int x=0;x <= codeIndex; x++) {
            Serial.print(programData[x]);
            addComma();
            if ((x > 0) && (x % 30 == 0)) Serial.println();
        }
        Serial.println();
        Serial.println();
...
if (digitalRead(pinButton) == LOW) {
    ...
    calcFileChecksum();
    calcProgramChecksum();
    digitalWrite(pinMT, HIGH); // mute the buzzer
    loadProgram();
    digitalWrite(pinMT, LOW);// // unmute the buzzer
...
void loadProgram() {  
  ...
  for (int i = 0; i < 2; i++) {
    ...
    sendBit(1);
    Serial.println();
    Serial.println(F("Load process finished"));
    Serial.println();
    Serial.println(F("When ready, 1. run LLIST or program with LPRINT or 2. send basic source file via serial input"));
    Serial.println();
} 
